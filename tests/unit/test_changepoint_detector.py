...
@pytest.fixture
def dummy_provenance() -> ChangePointProvenanceIdentity:
...
def test_negative_drift_reset_and_active_run_length(dummy_provenance):
    st = create_initial_changepoint_state()
    ...
    res = update_changepoint_detector(
        st, ev, st1, sequence_number=1, provenance=dummy_provenance, null_offset_d=10.0
    )
...
def test_positive_accumulation_and_onset_estimation(dummy_provenance):
    st = create_initial_changepoint_state()
    cfg = SessionConfig(noise_parameter_p=0.05, seed=12345)
    tr = run_session(cfg)
    ev = extract_evidence(tr)
    ...
    res1 = update_changepoint_detector(
        st, ev, st1, sequence_number=1, provenance=dummy_provenance, null_offset_d=0.0
    )
    ...
    res2 = update_changepoint_detector(
        st_next1, ev2, st1_2, sequence_number=2, provenance=dummy_provenance, null_offset_d=0.0, threshold=0.01
    )
...
def test_elevated_state_freeze_behavior(dummy_provenance):
    st = create_initial_changepoint_state()
    ...
    res1 = update_changepoint_detector(
        st, ev, st1, sequence_number=1, provenance=dummy_provenance, null_offset_d=0.0, threshold=0.001
    )
    ...
    res2 = update_changepoint_detector(
        st_elevated, ev2, st1_2, sequence_number=2, provenance=dummy_provenance, null_offset_d=0.0, threshold=0.001
    )
    assert res2.next_state == st_elevated
    assert "frozen" in res2.diagnostic_reason.lower()


def test_duplicate_and_out_of_order_rejection(dummy_provenance):
    st = create_initial_changepoint_state()
    tr = run_session(SessionConfig(noise_parameter_p=0.05, seed=100))
    ev = extract_evidence(tr)
    st1 = evaluate_stage1(ev)
    ...
    res_dup = update_changepoint_detector(
        st_active, ev, st1, sequence_number=2, provenance=dummy_provenance, null_offset_d=0.0
    )
    ...
    res_ooo = update_changepoint_detector(
        st_active, ev2, st1_2, sequence_number=1, provenance=dummy_provenance, null_offset_d=0.0
    )
