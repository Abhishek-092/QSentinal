...
def test_sequential_artifact_reproducibility(st1_artifact):
    a1 = generate_sequential_calibration_artifact(st1_artifact, monitoring_horizon_sessions=5, n_trials=20)
    a2 = generate_sequential_calibration_artifact(st1_artifact, monitoring_horizon_sessions=5, n_trials=20)
...
def test_different_horizon_alters_hash(st1_artifact):
    a1 = generate_sequential_calibration_artifact(st1_artifact, monitoring_horizon_sessions=5, n_trials=20)
    a2 = generate_sequential_calibration_artifact(st1_artifact, monitoring_horizon_sessions=10, n_trials=20)
...
def test_different_alpha_seq_alters_hash(st1_artifact):
    a1 = generate_sequential_calibration_artifact(st1_artifact, alpha_seq=0.01, n_trials=20)
    a2 = generate_sequential_calibration_artifact(st1_artifact, alpha_seq=0.05, n_trials=20)
...
def test_seed_range_validation():
    with pytest.raises(SeedAllocationError):
        SeedAllocator.get_seed("VALIDATION", 0)  # Cannot be used for calibration

    with pytest.raises(SeedAllocationError):
        SeedAllocator.get_seed("EVALUATION", 0)  # Cannot be used for calibration
...
def test_runtime_uses_sequential_artifact_threshold(st1_artifact, seq_artifact):
    ...
    calib_dec = evaluate_calibrated_stage1(stage1_res, st1_artifact)
    ...
    assert seq_artifact.empirical_sequential_threshold in res.diagnostic_reason
...
def test_stage1_provenance_mismatch_rejection(st1_artifact, seq_artifact):
    ...
    calib_dec_mismatched = evaluate_calibrated_stage1(stage1_res, st1_artifact)
...
def test_horizon_exhaustion_handling(st1_artifact, seq_artifact):
    ...
    calib_dec_11 = evaluate_calibrated_stage1(stage1_res_11, st1_artifact)
